...
class Driver_Setup:
	...
	def __init__(self):
		...
		self.connectivity, self.sim = self.tvb_connectivity(self.args.n_regions)
		self.weights = self.connectivity.weights
		self.lengths = self.connectivity.idelays
		...
		self.buf_len_ = self.connectivity.horizon + self.args.n_time
		self.buf_len = 2 ** np.argwhere(2 ** np.r_[:30] > self.buf_len_)[0][0]  # use next power of

		gf = self.sim.integrator.noise.gfun(None)
		self.r, self.V = self.sim.integrator.noise.generate(shape=(2, self.args.n_regions, self.buf_len)) * gf
	...
	def tvb_connectivity(self, tvbnodes):
		...
		sim.configure()
		# print(sim.connectivity.horizon)

		return white_matter, sim
...
class Driver_Execute(Driver_Setup):
	...
	def make_kernel(self, source_file, warp_size, args, lineinfo=False, nh='nh'):
		...
		try:
			with open(source_file, 'r') as fd:
				...
				mod_func = "{}{}{}{}".format('_Z', len(args.model), args.model, 'jjjjjfPfS_S_S_S_S_S_')
	...
	def run_simulation(self):
		...
		# setup data#{{{
		data = { 'weights': self.weights, 'lengths': self.lengths, 'params': self.params.T, 'noiser': self.r,
				 'noiseV': self.V}
			...
			for i in tqdm.trange(nstep, file=sys.stdout):
				...
				try:
					...
					step_fn(np.uintc(i * self.n_inner_steps), np.uintc(self.args.n_regions), np.uintc(self.buf_len),
							...
							gpu_data['noiser'], gpu_data['noiseV'], gpu_data['tavg%d' % (i%2,)],
	...
	def plot_output(self, tavg):
		plt.plot((tavg[:, self.args.plot_data, :, 0]))
		plt.show()
